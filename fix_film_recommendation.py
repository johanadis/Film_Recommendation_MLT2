# -*- coding: utf-8 -*-
"""FIX_film_recommendation.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1-LQdO3VcLiWB2DFgdnBsH-kFKxApilcL
...
1. Menghapus baris dengan nilai null pada kolom 'tagline' dan 'overview'
"""

df2.dropna(subset=['tagline', 'overview'], inplace=True)

"""2. Memeriksa kolom yang sama antara `df1` dan `df2` untuk menghindari duplikasi data."""
...
"""Menampilkan 10 Judul Film Teratas dari DataFrame df1"""

df1["title"].head(10)

"""Menampilkan 10 Judul Film Teratas dari DataFrame df2"""

df2["title"].head(10)

"""Terlihat bahwa isi pada kolom title pada df1 dan df2 sama, sehingga salah satu kolom dapat dihapus untuk menghindari redundansi data.

3. Menghapus kolom `title` dari `df2` karena kolom ini sudah ada di `df1` dengan nilai yang identik.
"""

del df2['title']

"""4. Menggabungkan `df1` dan `df2` menggunakan fungsi `pd.merge()` berdasarkan kolom `movie_id` di `df1` dan `id` di `df2`."""

df = pd.merge(df1, df2, left_on='movie_id', right_on='id')

"""4.1.  Menampilkan informasi dataset hasil penggabungan"""

df.info()

"""5. Menghapus kolom `id` pada dataset hasil penggabungan,, karena sudah ada `movie_id` dengan nilai yang sama."""

del df['id']

"""5.1.  Menampilkan 5 data  teratas"""
...
1. Mengambil data kolom `popularity` dari DataFrame.
2. Inisialisasi objek `MinMaxScaler` dari scikit-learn.
3. Melakukan fit dan transformasi data popularity agar berada pada rentang 0 hingga 1.

Normalisasi ini penting agar fitur numerik `popularity` memiliki skala yang sama dengan fitur lain, sehingga tidak mendominasi perhitungan kemiripan pada sistem rekomendasi.
"""
